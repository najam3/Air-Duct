    ...
    <header className='relative'>
        <nav style={{pointerEvents: isOpen ? 'none' : 'auto'}}>
            ...
            <div className={`${scrollPosition > 200 ? 'fixed shadow-2xl top-0 w-full z-50' : ''} flex items-center flex-col sm:flex-row gap-2 justify-center bg-neutral100 shadow-2xl py-4 pb-8 lg:pb-4 lg:pt-4`}>
                <button className='bg-secondary px-12 py-4 sm:text-xl text-white font-semibold w-1/2 sm:w-[260px]'>
                    <Link to={'/contact'}>
                        CALL US
                    </Link>
                    
                    </button>
                <button className='bg-primary px-12 py-4 sm:text-xl text-white font-semibold w-1/2 sm:w-[260px]'>
                        <Link to={'/contact'}>
                            SCEDHULE NOW
                        </Link>
                    </button>
